...
layout(rgba32f, set = 0, binding = 2) uniform restrict image2D output_image;
...
void main() {
	...
	vec4 color = textureBicubic(source_sampler, pos.xz);
	vec4 original = imageLoad(output_image, coords);

	vec4 result;
	if(source.blend_mode == 0){
		result = vec4(color.rgb, 1);
	}
	else if(source.blend_mode == 1) {
		result = vec4((original + color).rgb, 1);
	}
	else if(source.blend_mode == 2) {
		result = vec4(min(original, color).rgb, 1);
	}
	else if(source.blend_mode == 3) {
		result = vec4(max(original, color).rgb, 1);
	}
